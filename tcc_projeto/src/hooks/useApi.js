...
export function useVerificaLogin() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        for (let i = 0; i < 2; i++) {
          const userType = i === 0 ? "solicitante" : "administrador";

          const request = await fetch(`${url}/${userType}`);
          const response = await request.json();

          i === 0 ? setSolicitantes(response) : setAdministradores(response);
        }
      } catch (error) {
...
export function useDemanda() {
  ...
  const cadastrarDemanda = async (data) => {
    ...
    const isIdSolicitanteDefinido = await defineIdSolicitante(
      "demanda",
      response.id
    );
